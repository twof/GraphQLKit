...
struct _GraphQLQueryKeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
    typealias Key = K
    ...
    init(referencing encoder: __GraphQLQueryEncoder, codingPath: [CodingKey], wrapping container: NSMutableDictionary) {
    ...
    private func _converted(_ key: CodingKey) -> CodingKey {
        ...
        case .convertToSnakeCase:
            let newKeyString = GraphQLQueryEncoder.KeyEncodingStrategy._convertToSnakeCase(key.stringValue)
            return _GraphQLQueryKey(stringValue: newKeyString, intValue: key.intValue)
    ...
    public mutating func encodeNil(forKey key: Key) throws {
        self.container[_converted(key).stringValue] = [:]
    }
    ...
    public mutating func superEncoder() -> Encoder {
        return __GraphQLQueryReferencingEncoder(referencing: self.encoder, key: _GraphQLQueryKey.super, convertedKey: _converted(_GraphQLQueryKey.super), wrapping: self.container)
    }

    public mutating func superEncoder(forKey key: Key) -> Encoder {
        return __GraphQLQueryReferencingEncoder(referencing: self.encoder, key: key, convertedKey: _converted(key), wrapping: self.container)
    }
}
